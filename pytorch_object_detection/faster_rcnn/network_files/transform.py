...
def _resize_image(image, self_min_size, self_max_size):
    ...
    image = torch.nn.functional.interpolate(  # 在image最前加一个新维度: [1,C,H,W]
        image[None], scale_factor=scale_factor, mode="bilinear", recompute_scale_factor=True, # 双线性插值方法，只支持4Dtensor
        align_corners=False)[0]  # 用切片的方式转化为原本的shape

    return image


class GeneralizedRCNNTransform(nn.Module):  # 继承nn.Module的类
    """
    ...
    def __init__(self, min_size, max_size, image_mean, image_std):
        ...
        if not isinstance(min_size, (list, tuple)):  # 转化成tuple类
            min_size = (min_size,)
    ...
    def normalize(self, image):
        """标准化处理"""
        dtype, device = image.dtype, image.device  # 获得类型与设备信息
        mean = torch.as_tensor(self.image_mean, dtype=dtype, device=device)
        std = torch.as_tensor(self.image_std, dtype=dtype, device=device)
        # [:, None, None]: shape [3] -> [3, 1, 1] 因为图像是[C,W,H]三维，因此需要其与image维度相同，通过添加none维度
        return (image - mean[:, None, None]) / std[:, None, None]  # 减去均值除以方差
    ...
    def resize(self, image, target):  # 缩放
        ...
        if target is None:  # 如果是空则是验证模式
            return image, target

        bbox = target["boxes"]  # 对box进一步缩放
        # 根据图像的缩放比例来缩放bbox
        bbox = resize_boxes(bbox, [h, w], image.shape[-2:])  # 缩放前原始h,w / 缩放后image的H，W
        target["boxes"] = bbox
    ...
    def batch_images(self, images, size_divisible=32):  # 保持图像正常比例，图像左上角对齐，其余少于最大图片的部分都补0，使之与最大图片大小相同
        # type: (List[Tensor], int) -> Tensor
        """
        ...
        if torchvision._is_tracing():  # 永远不满足
            # batch_images() does not export well to ONNX
            # call _onnx_batch_images() instead
            return self._onnx_batch_images(images, size_divisible)  # 转化为onnx模型，开放神经网络交换格式，在PT，TF之间转换
...
def resize_boxes(boxes, original_size, new_size):
    ...
    """
    ratios = [
        torch.tensor(s, dtype=torch.float32, device=boxes.device) /  # 新尺寸/旧尺寸得到缩放因子
        torch.tensor(s_orig, dtype=torch.float32, device=boxes.device)
        for s, s_orig in zip(new_size, original_size)
    ]
    ratios_height, ratios_width = ratios
    # Removes a tensor dimension, boxes [minibatch, 4] 当前图片中有几个box信息、xmin ymin xmax ymax
    # Returns a tuple of all slices along a given dimension, already without it.
    xmin, ymin, xmax, ymax = boxes.unbind(1)
    xmin = xmin * ratios_width
    xmax = xmax * ratios_width  # 宽度方向
